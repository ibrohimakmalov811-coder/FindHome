  ...
  return (
    <div className="w-full max-w-[1720px] mx-auto px-4 sm:px-8 lg:px-12 xl:px-16 py-8 sm:py-12">
      ...
      <div className="space-y-6 mb-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-3 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center gap-3 transition-colors">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && filters.searchQuery.trim().toLowerCase() === 'admin') {
                  router.push('/admin');
                }
              }}
        ...
        {filters.searchQuery.trim().toLowerCase() === 'admin' && (
          <div className="w-full p-3.5 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 rounded-2xl flex items-center justify-between text-xs text-indigo-900 dark:text-indigo-200 animate-in fade-in">
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-5 h-5 text-indigo-600 shrink-0" />
              <div>
                <span className="font-bold block">Administrator Paneli</span>
                <span className="text-[11px] opacity-80">Boshqaruv tizimiga kirish uchun quyidagi tugmani bosing</span>
              </div>
            </div>
            <Link
              href="/admin"
              className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md transition-all flex items-center gap-1.5 shrink-0"
            >
